...
def get_original_player_with_metrics(uniform_number, user):
    """
    ログインユーザーが作成した選手について、
    背番号を基点に選手情報・成績情報・指標情報をまとめて取得するサービス。

    - userに紐づく選手のみが対象
    - 打者/投手の判定および指標計算は本サービス層で完結
    - 該当選手が存在しない場合はNoneを返す
    """
    result = getter["user_player_by_uniform_number"](uniform_number, user)
    if not result:
        return None
